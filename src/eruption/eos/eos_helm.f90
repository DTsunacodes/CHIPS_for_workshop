...
! helmholtz.f90 was modified for the sake of determinating P,T using e,rho,X
! modified by Kuriyama (2018/10/30)
...
! zion    = number of protons in isotope i
      ...
      integer          ionmax, k, j, mn, nelem, done
      parameter        (ionmax=19)
      double precision xmass(ionmax),aion(ionmax),zion(ionmax),temp,den,abar,zbar
! define initial trial temperature
      double precision temp_trial,e_th
! epsilon decide when to stop
      double precision epsilon_temp
...
! set the mass fractions, z's and a's of the composition
      ...
      xmass(1) = 0.75d0 ; aion(1)  = 1.0d0  ; zion(1)  = 1.0d0  !H1
      xmass(2) = 0.0d0 ; aion(2)  = 3.0d0  ; zion(2)  = 2.0d0   !He3
      xmass(3) = 0.23d0 ; aion(3)  = 4.0d0  ; zion(3)  = 2.0d0  !He4
      xmass(4) = 0.02d0 ; aion(4)  = 12.0d0 ; zion(4)  = 6.0d0  !C12
      xmass(5) = 0.0d0 ; aion(5)  = 14.0d0 ; zion(5)  = 7.0d0  !n14
      xmass(6) = 0.0d0 ; aion(6)  = 16.0d0  ; zion(6)  = 8.0d0  !O16
      ...
      xmass(19) = 0.0d0 ; aion(19)  = 56.0d0 ; zion(19)  = 28.0d0 !Ni56
...
! set the input vector. pipeline is only 1 element long in this example
       ...
       e_th = e_ar(k) -0.5d0*u_ar(k)**2 -grv_ar(k)*rn


       thnegative = 0
       if(e_th.lt.0.0d0)then
         write(*,*)"e_th negative at",k
         e_th = 1.d-3*u_ar(k)**2
         thnegative = 1
       end if

       den_row(1)  = 1.d0/tau_ar(k)
       abar_row(1) = abar
       zbar_row(1) = zbar
       jlo_eos = 1 ; jhi_eos = 1
...
! end of calculation
       ...
       p_ar(k) = ptot_row(1)
       if(thnegative.eq.0)eu_ar(k) = e_th
